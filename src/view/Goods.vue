<template>
	<div class="goods"  style="position: relative; height:100vh; overflow: hidden; width: 100%;
		overflow-y: scroll;" @scroll="scrollInfo($event)">
		<div class='product_view'>
		    <img class="left" src="src/assets/images/left.png" alt="">
			<slider :slider_img="goods_imgs"> </slider>
		...
		<div class="ui_line fouritem_line">	
		</div>
		<div>
		<div :class="cur_info==0?'fouritem':'fouritem fouritem_f'" >
			<ul>
				<li><router-link :to="'/goods/'+goods_info.id"> 概述 </router-link></li>
				<li><router-link :to="'/goods/'+goods_info.id+'/goodsParam'"> 参数 </router-link></li>
				<li><router-link :to="'/goods/'+goods_info.id+'/goodsInsure'"> 意外险 </router-link></li>
			</ul>
		</div>
		<router-view/> 
		...
		<div class="cartFoot_menu_box" v-if="cartFoot==false">
			<input type="button" value="确定" class="add_cart_button" style="width: 100%;" @click="confirm()">
		</div>
	...
	export default {
        data(){
            return {
                cartFoot:true,
                goods_imgs: [
					require('../assets/images/goods_04.png'),
					require('../assets/images/goods_05.png'),	
					require('../assets/images/goods_06.png'),
					require('../assets/images/goods_04.png'),
					require('../assets/images/goods_05.png')			
				],
				goods_info:[],
        ...
        methods:{
        	init:function () {				
				axioss.post('/goods/ajaxInfo',{				
					id:this.$route.params.id
				}).then((rtnData)=>{
					this.goods_info = rtnData.data;
				})
			},
			scrollInfo:function (event) {
				let cur_scroll_y = event.target.scrollTop;
				var f_height=document.querySelector(".fouritem_line").offsetTop;
                  if (cur_scroll_y > f_height){
				 	this.cur_info=1
				 }else {
				 	this.cur_info=0
				 }				
            ...
            confirm:function () {
            	this.goods_info.buy_num = 1
            	lockr.sadd('cart_list',this.goods_info)
